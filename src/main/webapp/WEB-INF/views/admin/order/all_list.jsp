...
<body>
<form id="form" action="allList.do" method="post">
			...
			<table cellspacing=0 cellpadding=3 width="100%" align=center border=0>
				<tbody>
					<tr class=list>
						...
						<td width="100" height=28 class=biaoti><span
							class="searchzone"><spring:message code="order.label.status"/></span></td>
						...
						<tr bgcolor="#FFFFFF" onmouseover="this.bgColor='#f2f9fd'" onmouseout="this.bgColor='#FFFFFF'" id="order${order.id}">
							...
							<td class=content>
								<c:if test="${order.status==1}">
									<spring:message code="order.label.query.peddingsend"/>
								</c:if>
								<c:if test="${order.status==2}">
									<spring:message code="order.label.query.sendtoborder"/>
								</c:if>
								<c:if test="${order.status==3}">
									<spring:message code="order.label.query.arriveborder"/>
								</c:if>
								<c:if test="${order.status==4}">
									<spring:message code="order.label.query.hadget"/>
								</c:if>
								<c:if test="${order.status==5}">
									<spring:message code="order.label.query.sendtohn"/>
								</c:if>
								<c:if test="${order.status==6}">
									<spring:message code="order.label.query.sendtohcm"/>
								</c:if>
								<c:if test="${order.status==7}">
									<spring:message code="order.label.query.goodsinware"/>
								</c:if>
								<c:if test="${order.status==8}">
									<spring:message code="order.label.query.customergetgoods"/>
								</c:if>
								<c:if test="${order.status==9}">
									<spring:message code="order.label.query.getmoney"/>
								</c:if>
								<c:if test="${order.status==10}">
									<spring:message code="order.label.query.finish"/>
								</c:if>
							</td>
