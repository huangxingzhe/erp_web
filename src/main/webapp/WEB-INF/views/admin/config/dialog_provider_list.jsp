	...
	<table height=30 cellspacing=0 cellpadding=0 width="100%" border=0>
			...
			<tr>
				<td>
					...
					<select name="type" id="type">
			    		...
			    		<option value="3" <c:if test="${type==3}">selected</c:if>>地址</option>
			    	</select>
			    	<input type="text" value="${keyword}" name="keyword" id="keyword"/>
			    	<input onclick="location.href='list.do'"type="submit" value="查询" class="button" />
				</td>
			</tr>
		</tbody>
		</table>
		...
		<div class=listzone>
			<table cellspacing=0 cellpadding=3 width="100%" align=center border=0>
				<tbody>
					<tr class=list>
						<td width="15%" height=20 class=biaoti style="padding:0xp 0px;"><span
							class="searchzone"><spring:message code="provider.label.code"/></span></td>
						<td width="40%" height=20 class=biaoti><span
							class="searchzone"><spring:message code="provider.label.name"/></span></td>
						<td width="45%" height=20 class=biaoti><span
							class="searchzone"><spring:message code="provider.label.address"/></span></td>
					</tr>
					<c:forEach items="${providers}" var="provider" >
						<tr onclick="parent.window.selectItem('providerName','${provider.id}','${provider.code}','${provider.name}')" style="cursor:pointer;"
						bgcolor="#FFFFFF" onmouseover="this.bgColor='#f2f9fd'" onmouseout="this.bgColor='#FFFFFF'" id="provider${provider.id}">
