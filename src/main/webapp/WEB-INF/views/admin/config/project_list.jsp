			...
			<table cellspacing=0 cellpadding=3 width="100%" align=center border=0>
				<tbody>
					...
					<c:forEach items="${projects}" var="project" >
						<tr bgcolor="#FFFFFF" onmouseover="this.bgColor='#f2f9fd'" onmouseout="this.bgColor='#FFFFFF'" id="project${project.id}">
							<td class=content>&nbsp;</td>
							<td class=content>${project.name}</td>
							<td class=content>
								<c:if test="${project.status==1}">正常</c:if>
				    			<c:if test="${project.status==2}">取消</c:if>
							</td>
							<td class=content id="opt_${project.id}">
							<c:if test="${update!=null}">
							<a href="javascript:edit(${project.id})"><spring:message code="admin.label.edit"/></a>
							</c:if>
							<c:if test="${delete!=null}">
								&nbsp;&nbsp;<a href="javascript:DelRow(${project.id})"><spring:message code="admin.label.delete"/></a>
							</c:if>
							</td>
						</tr>
					</c:forEach>
