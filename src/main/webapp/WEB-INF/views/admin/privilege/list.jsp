			...
			<tbody>
				<tr>
					<td align=right colspan=3><input
						onclick="javascript:location.href='init.do?id=0'" type="button" value="添加"
				...
				<tbody>
					<tr class=list>
						...
						<td width="100" height=28 class=biaoti><span
							class="searchzone">权限名称</span></td>
						<td width="100" height=28 class=biaoti><span
							class="searchzone">权限值</span></td>
						<td class=biaoti width=50><span class="searchzone">操作</span></td>
					</tr>
					<c:forEach items="${privileges}" var="pri" >
					<tr bgcolor="#FFFFFF" onmouseover="this.bgColor='#f2f9fd'" onmouseout="this.bgColor='#FFFFFF'" id="user${pri.id}">
						<td class=content><input type="checkbox"  value="${pri.id}" name="ids"></td>
						<td class=content>${pri.name}</td>
						<td class=content>${pri.value}</td>
						<td class=content id="opt_${pri.id}"><a href="javascript:edit(${pri.id})">修改</a>
						&nbsp;&nbsp;<a href="javascript:DelRow(${pri.id})">删除</a>
						</td>
					</tr>
					</c:forEach>
				</tbody>
